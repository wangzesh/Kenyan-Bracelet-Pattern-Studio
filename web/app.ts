...
const NAMED_PREVIEW_COLORS: Record<string, string> = {
  black: "#111111",
...
const patternInput = byId<HTMLTextAreaElement>("patternInput");
const fileInput = byId<HTMLInputElement>("fileInput");
const openPatternButton = byId<HTMLButtonElement>("openPatternButton");
const lengthInput = byId<HTMLInputElement>("lengthInput");
const heightsInput = byId<HTMLInputElement>("heightsInput");
...
const issuesList = byId<HTMLUListElement>("issues");
...
byId<HTMLButtonElement>("sampleButton").addEventListener("click", () => {
  patternInput.value = DEFAULT_PATTERN;
  render();
});

byId<HTMLButtonElement>("copyButton").addEventListener("click", () => {
  copyPattern();
});

byId<HTMLButtonElement>("downloadPatternButton").addEventListener("click", () => {
...
byId<HTMLButtonElement>("downloadSvgButton").addEventListener("click", () => {
  const parsed = parsePattern(patternInput.value);
...
byId<HTMLButtonElement>("addColumnButton").addEventListener("click", () => {
  const parsed = parsePattern(patternInput.value);
...
byId<HTMLButtonElement>("removeColumnButton").addEventListener("click", () => {
...
function parsePaletteLine(line: string, lineNumber: number, issues: Issue[]): PaletteEntry | null {
...
function parsePatternLine(line: string, lineNumber: number, index: number, paletteSymbols: string[]): ColumnEntry | null {
...
function parseBeads(beadsText: string, paletteSymbols: string[]): string[] {
...
function parseHeights(value: string, issues: Issue[]): number[] {
...
function parseAlign(value: string, issues: Issue[]): AlignMode {
...
function defaultOrientation(heights: number[]): OrientationMode {
...
function syncControls(parsed: ParsedPattern): void {
...
function renderStats(parsed: ParsedPattern): void {
...
function renderIssues(parsed: ParsedPattern): void {
  issuesList.replaceChildren();
  const issues = parsed.issues.length > 0 ? parsed.issues : [{ level: "ok" as IssueLevel, message: "All counts valid." }];
...
function renderLegend(parsed: ParsedPattern): void {
...
function renderSvg(parsed: ParsedPattern): void {
...
function columnYOffset(align: AlignMode, maxHeight: number, columnHeight: number, pitchY: number): number {
...
function setMeta(text: string, key: string, value: string): string {
...
function resizePatternColumns(text: string, targetLength: number): string {
...
function makeColumnLine(parsed: ParsedPattern, index: number): string {
...
function paintBead(parsed: ParsedPattern, columnIndex: number, beadOffset: number): void {
...
function replaceBeadAt(text: string, parsed: ParsedPattern, columnIndex: number, beadOffset: number, symbol: string): string {
...
function normalizeLength(value: string, fallback: number): number {
  const parsed = Number(value);
  if (!Number.isFinite(parsed) || parsed < 1) return Math.max(1, fallback);
  return Math.floor(parsed);
}

function removeLastPatternLine(text: string): string {
...
function downloadText(name: string, type: string, text: string): void {
